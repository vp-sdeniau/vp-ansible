...
def add_repository_build_type(facts, key, host_variables, failure_msgs):
    ...
    if host_family in support_map:
        supported_releases = support_map[host_family]
        ...
        if os_major_version in supported_releases:
            ...
            if requested_repo_format in repository_build_types:
                ...
                failure_msgs.append("Unsupported %s %s repository format: %s" % (host_family, os_major_version, requested_repo_format))
        else:
            failure_msgs.append("Unsupported %s version: %s" % (host_family, os_major_version))
    else:
        failure_msgs.append("Unsupported operating system family: %s" % host_family)
